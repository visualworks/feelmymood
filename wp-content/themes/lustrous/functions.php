...
if ( ! function_exists( 'lustrous_setup' ) ) :

/** Sets up theme defaults and registers support for various WordPress features. */
function lustrous_setup() {
	/* Make theme available for translation
	  Translations can be filed in the /languages/ directory	 */
	load_theme_textdomain( 'lustrous', get_template_directory() . '/languages' );
	...
	register_nav_menus( array(
		'main-menu' => __( 'Main Menu', 'lustrous' ),
		'footer-menu' => __( 'Footer Menu', 'lustrous' ),
	) );
	
	global $content_width;
 if ( ! isset( $content_width ) ) { $content_width = 800; /* pixels */ }
}
endif; // lustrous_setup
add_action( 'after_setup_theme', 'lustrous_setup' );
