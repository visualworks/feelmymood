...
class MK_Typography_Control extends MK_Control {
		...
		<div class="mk-control-wrap mk-control-typography">
			<div class="mk-control-typography-inner">
				...
				<div class="mk-row">
					...
					<div class="mk-col-3">
						<div class="mk-typography-item mk-typography-item-color">
							<label>
							<?php
		...
		if ( $safe_fonts ) {
			foreach ( $safe_fonts as $safe_font ) {
				$all_fonts[ $safe_font ] = array(
					'label' => $safe_font,
					'data-source' => 'safe-font',
				);
			}
		}

		$google_fonts = array_keys( $this->google_fonts( true ) );
		if ( $google_fonts ) {
			foreach ( $google_fonts as $google_font ) {
				$all_fonts[ $google_font ] = array(
					'label' => $google_font,
					'data-source' => 'google-font',
				);
			}
		}
	 ...
	 *
	 * @param bool $assoc_array Set the returned data format as object or assoc array.
	 * @return object|array
	 */
	private function google_fonts( $assoc_array ) {

		if ( ! isset( $assoc_array ) ) {
			$assoc_array = false;
		}

		$json_font_file = THEME_CUSTOMIZER_DIR . '/controls/' . $this->type . '/google-fonts.json';

		$mkfs = new Mk_Fs( array(
			'path' => $json_font_file,
		) );

		$response = $mkfs->get_contents( $json_font_file );

		if ( $mkfs->get_error_codes() || ! $response ) {
			return array();
		}

		return json_decode( $response, $assoc_array );
	}
}
