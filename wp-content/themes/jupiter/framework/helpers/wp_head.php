...
if ( ! function_exists( 'mk_open_graph_meta' ) ) {
	function mk_open_graph_meta() {
		...
		if ( ! is_single() || defined( 'WPSEO_VERSION' ) ) { return false;
		}
		...
		$output .= '<meta property="og:title" content="' . the_title_attribute( array(
			'echo' => false,
		) ) . '"/>' . "\n";
...
if ( ! function_exists( 'mk_get_header_class' ) ) {
	function mk_get_header_class( $atts = array() ) {
		...
		// Disable toolbar when none of its elements are active.
		if ( 'true' !== $mk_options['enable_header_date'] &&
			...
			'toolbar' !== $mk_options['header_search_location'] &&
